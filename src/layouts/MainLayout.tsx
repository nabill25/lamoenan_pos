    ...
    <div className="flex h-[100dvh] bg-gray-50">
      ...
      <aside className="hidden md:flex md:w-64 bg-slate-900 text-white flex-col shrink-0">
        ...
        <div className="border-t border-slate-800 p-3 space-y-1">
          ...
          <button
            ...
            className={`w-full flex items-center gap-2 p-2 rounded-lg transition-colors text-sm ${activeShift
              ? 'text-green-400 hover:bg-slate-800'
              : 'text-slate-400 hover:bg-slate-800 hover:text-white'
              }`}
          >
            {activeShift ? <Unlock size={16} className="shrink-0" /> : <Lock size={16} className="shrink-0" />}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="md:hidden flex items-center justify-between px-4 py-3 bg-slate-900 text-white shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="font-bold text-sm leading-tight uppercase">Lamoenan</div>
              {badge && <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold ${badge.cls}`}>{badge.label}</span>}
            </div>
          </div>
          <div className="flex items-center gap-2">
            {/* Tombol shift di mobile */}
            <button
              onClick={() => setShowShiftModal(true)}
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-bold ${activeShift ? 'bg-green-600 text-white' : 'bg-slate-700 text-slate-300'}`}
            >
              {activeShift ? <><Clock3 size={12} /> Shift<span className="w-1.5 h-1.5 rounded-full bg-green-300 animate-pulse ml-0.5" /></> : <><Lock size={12} /> Buka Shift</>}
            </button>
      ...
      {showShiftModal && (
        <ShiftModal
          ...
          onShiftChange={shift => setActiveShift(shift as any)}
        />
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowLogoutModal(false)} className="p-1 text-gray-400 hover:text-gray-600 rounded"><X size={18} /></button>
            </div>
            <p className="text-sm text-gray-500 mb-6">Yakin ingin keluar? Anda perlu login kembali untuk mengakses aplikasi.</p>
            <div className="flex gap-3">
              <button onClick={() => setShowLogoutModal(false)} className="flex-1 px-4 py-2 border border-gray-200 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50">Batal</button>
              <button onClick={handleLogout} className="flex-1 px-4 py-2 bg-red-500 text-white rounded-lg text-sm font-bold hover:bg-red-600">Ya, Keluar</button>
            </div>
          </div>
        </div>
      )}
